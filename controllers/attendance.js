...
const DEFAULT_END_TIME = 23; // 6:00 PM
 ...
 const checkIn = async (req, res, next) => {
  try {
    ...
    // startWorking.setUTCHours(DEFAULT_START_TIME); 
    console.log("startWorking:",startWorking.getHours())
    startWorking.setHours(startWorking.getHours() + 2);
    console.log("startWorking:",startWorking.getHours())
    console.log("attendances.checkIn.:",attendances[0].checkIn.getHours())
    ...
    if (attendances[0].checkIn.getTime() >= startWorking.getTime()) {
      ...
      if (lateArrivals == 1) {
        ...
        // console.log("attendances.employee",attendances.employee);
  
        // const attendances[0] =attendances.employee;
        if (attendances[0].lateCounter == 0) {
          console.log("attendances[0]",attendances[0]);
        ...
        let deduction;
        // const employeeData = await Employee.findById(employee);
        // console.log(`attendance: ${attendances[0]},attendances: ${attendances}`);
        if (attendances[0].lateCounter == 0) {
            attendances[0].lateCounter = 1;
          // console.log('tttttt')
        } 
...
const checkOut = async (req, res, next) => {
  try {
    ...
    // console.log(`earlThreshold ${earlThreshold}`)
    ...
    // console.log(`"attendance out:"${attendance}`);
    // console.log(earlThreshold)
    // console.log(attendances.filter((a) => a.checkOut.getTime() >= earlThreshold.getTime()))
    ...
    // startWorking.setUTCHours(DEFAULT_START_TIME); 
    console.log("endWorking:",endWorking.getHours())
    endWorking.setHours(endWorking.getHours() - 2);
    console.log("endWorking:",endWorking.getHours() - 2)
    console.log("attendances.checkout:",attendances[0].checkOut.getHours())
    ...
    // count the number of early leaves
    console.log(`earlyLeaves ${earlyLeaves}`)
    console.log(earlThreshold);
    // console.log(attendance);
    // console.log("attendances[0]",attendances[0])
    if (earlyLeaves == 0) {
      // First early leaves - no deduction but issue a warning
      // Increment the late counter

      // const employeeData = await Employee.findById(employee);
      // const attendances[0] =attendance;
      if (attendances[0].lateCounter == 0) {
        // console.log("attendances[0].lateCounter:",attendances[0].lateCounter);
